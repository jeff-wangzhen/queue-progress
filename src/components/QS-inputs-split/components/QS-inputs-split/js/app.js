...
const getPickerAsyncData = async function(obj) {
  // picker-custom与picker-custom2的逐级获取数据函数
  let _this = this;
    ...
    switch (linkageNum) {
      ...
      case 3: // 三级联动
        if (launch) {
          // 首次初始化 value为传进来的defaultValue
          // 获取第二列数据
          sendData = getSendData({ ...obj, column: 0 }); // 访问接口携带参数
          const newData_1 = await getAsyncDataFc(sendData); // 从接口获取第二列的数据
          ...
          sendData = getSendData({
            ...obj,
            datas: { ...datas, data_1: [...newData_1] },
            column: 1,
          }); // 访问接口携带参数
          const newData_2 = await getAsyncDataFc(sendData);
          _this.async_2 = [...newData_2]; // 给第三列赋值
          if (!_this.memoryData_2[value[0]]) _this.memoryData_2[value[0]] = [];
...
const _app = {
  ...
  previewImage(imgPath, currentIndex) {
    if (!(imgPath instanceof Array)) imgPath = [imgPath];
    uni.previewImage({
      urls: imgPath,
  ...
  getForm(name) {
    const { status, result } = QSFormObj.get({
      name,
    });
    if (status === "success") {
      ...
      for (const o in obj) {
        ...
        switch (type) {
          case typeObj.input: //input
            ...
            if (
              verifyType &&
              verifyTypeObj[verifyType] !== undefined &&
              verifyTypeObj[verifyType].reg &&
              !verifyTypeObj[verifyType].reg.test(value)
            ) {
              if (verifyItem.result) verifyItem.result = false;
          ...
          case typeObj.pics: //pics
            if (isArrayAndHasLength(value)) {
              const notPathArr = value.filter(
                (item) => !item.path && item.required
              );
