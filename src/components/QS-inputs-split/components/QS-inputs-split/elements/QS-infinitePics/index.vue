<template>
  ...
  >
    <view
      class="width100 padding_10rpx_15rpx wrap"
      :class="itemLayout_computed"
    >
      <view
      ...
      >
        ...
        >
          ...
          >
            <uni-icon
              type="clear"
              :color="picsItem.clearColor || clearColor || '#f5105c'"
              :size="34"
            />
          </view>
          <view
  ...
  methods: {
    ...
    clearPic({
      ...
      if (this.sortSelection) {
        const selectionIndex = this.sortSelectionCheckArray.findIndex(
    ...
    showImg({
      ...
      _app.previewImage(
        this.itemArray.map((item) => item.path),
        picsindex
      );
    },
    sortSelectionFc({
      ...
      const hasIndex = this.sortSelectionCheckArray.findIndex(
        (item) => item.path === this.itemArray[picsindex].path
      );
      if (hasIndex >= 0) {
    ...
    checkPicsCheckObj(vbName) {
      ...
      for (let i = 0; i < itemArray.length; i++) {
        const index = sortSelectionCheckArray.findIndex(
          (ite) => ite.path === itemArray[i].path
        );
        if (index >= 0) {
