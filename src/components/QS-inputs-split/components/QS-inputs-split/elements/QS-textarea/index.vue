      ...
      <view class="Flex5 position_relative" :class="itemLayout_computed">
        <textarea
          ...
          :style="{
            ...
            'border':
              '1px solid ' + (focusBl ? focusBorderColor : blurBorderColor),
          }"
  ...
  methods: {
    inputs_change({ detail: { value } } = {}) {
      ...
      if (this.inputDebounceSet.openInputDebounce) {
        ...
        if (
          inputDebounce.debounceTime &&
          new Date().getTime() - inputDebounce.debounceTime <
    ...
    inputs_changeFc(value, clash) {
      ...
      if (this.filterFc && typeof this.filterFc == "function") {
        //有filterFc则过滤
        this.input_filter_change(value, this.filterFc);
      } else if (
        this.filterType &&
        _app.filterTypeObj[this.filterType] &&
        typeof _app.filterTypeObj[this.filterType] == "function"
      ) {
