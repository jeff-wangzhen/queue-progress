      ...
      >
        <block v-if="mode === 'bottom'">
          ...
          >
            ...
            <view
              class="flex_row_c_c width250rpx"
              :style="{ color: titleColor || '#999' }"
            >
              {{ title || "" }}
            </view>
            <view
        ...
        <picker-view
          @touchmove.prevent.stop="voidFc()"
          :indicator-style="
            'height:' + wH * (lineHeight || 0.09) + 'px;' + indicator_style
          "
          class="width100 height100 backgroundColor_white"
          :style="{
        ...
        >
          <block v-if="type === 'date'">
            ...
            <picker-view-column v-if="setObj.dateMode >= 2">
              ...
              <view
                class="flex_row_c_c"
                v-for="(picker_item, picker_index) in 12"
                :key="picker_index"
              >
                {{ picker_item + 1 }}月
              </view>
            ...
            <picker-view-column v-if="setObj.dateMode >= 4">
              ...
              <view
                class="flex_row_c_c"
                v-for="(picker_item, picker_index) in 24"
                :key="picker_index"
              >
                {{ picker_item }}时
              </view>
            ...
            <picker-view-column v-if="setObj.dateMode >= 5">
              ...
              <view
                class="flex_row_c_c"
                v-for="(picker_item, picker_index) in 60"
                :key="picker_index"
              >
                {{ picker_item }}分
              </view>
          ...
          <block v-else-if="type === 'city'">
            <picker-view-column>
              <view
                class="flex_row_c_c"
                v-for="(picker_item, picker_index) in provinceDataList"
                :key="picker_index"
              >
                {{ picker_item.label }}
              </view>
            ...
            <picker-view-column>
              <view
                class="flex_row_c_c"
                v-for="(picker_item, picker_index) in cityDataList"
                :key="picker_index"
              >
                {{ picker_item.label }}
              </view>
            ...
            <picker-view-column>
              <view
                class="flex_row_c_c"
                v-for="(picker_item, picker_index) in areaDataList"
                :key="picker_index"
              >
                {{ picker_item.label }}
              </view>
          ...
          <block v-else-if="type === 'provincialStreet'">
            ...
            <picker-view-column>
              <view
                class="flex_row_c_c"
                v-for="(picker_item, picker_index) in streetDataList"
                :key="picker_index"
              >
                {{ picker_item }}
              </view>
          ...
          <block v-if="type === 'custom'">
            <block v-if="setObj.linkage">
              <block v-if="setObj.linkageNum === 2">
                <picker-view-column>
                  <view
                    class="flex_row_c_c"
                    v-for="(item, index) in setObj.itemArray"
                    :key="index"
                  >
                    {{ item[setObj.steps.step_1_value] }}
                  </view>
                </picker-view-column>
                <picker-view-column>
                  <view
                    class="flex_row_c_c"
                    v-for="(item, index) in setObj.itemArray[value[0]][
                      setObj.steps.step_2_item
                    ]"
                    :key="index"
                  >
                    {{ item[setObj.steps.step_2_value] || item }}
                  </view>
                </picker-view-column>
              </block>
              <block v-else-if="setObj.linkageNum === 3">
                ...
                <picker-view-column>
                  <view
                    ...
                    :key="index"
                  >
                    {{ item[setObj.steps.step_2_value] }}
                  </view>
                </picker-view-column>
                <picker-view-column>
                  <view
                    ...
                    v-for="(item, index) in setObj.itemArray[value[0]][
                      setObj.steps.step_2_item
                    ][value[1]][setObj.steps.step_3_item]"
                    :key="index"
                  >
                    {{ item[setObj.steps.step_3_value] || item }}
                  </view>
                </picker-view-column>
              </block>
            </block>
            <block v-else>
              <picker-view-column
                v-for="(items, indexs) in setObj.itemArray"
                :key="indexs"
              >
                <view
                  class="flex_row_c_c"
            ...
            <block v-if="setObj.linkage">
              <block v-if="setObj.linkageNum === 2">
                <picker-view-column>
                  <view
                    class="flex_row_c_c"
                    v-for="(item, index) in setObj.itemObject.step_1"
                    :key="index"
                  >
                    {{
                      setObj.steps.step_1_value
                ...
                <picker-view-column>
                  <view
                    class="flex_row_c_c"
                    v-for="(item, index) in setObj.itemObject.step_2[value[0]]"
                    :key="index"
                  >
                    {{
                      setObj.steps.step_2_value
                        ? item[setObj.steps.step_2_value]
                        : item
                    }}
                  </view>
              ...
              <block v-else-if="setObj.linkageNum === 3">
                ...
                <picker-view-column>
                  <view
                    class="flex_row_c_c"
                    v-for="(item, index) in setObj.itemObject.step_3[value[0]][
                      value[1]
                    ]"
                    :key="index"
                  >
                    {{
                      setObj.steps.step_3_value
                        ? item[setObj.steps.step_3_value]
                        : item
                    }}
                  </view>
                </picker-view-column>
...
export default {
  ...
  data() {
    return {
      ...
      wH,
      classObj: {
        btnSize: "font-size: " + wH * (this.fontscale + 0.005) + "px;",
      },
    };
  },
  computed: {
    heightSize() {
      if (this.mode === "middle") {
        return wH * (this.height || 0.3) + "px";
      } else {
        return wH * (this.height || 0.45) + "px";
      }
    },
    titleSize() {
      return wH * (this.fontscale + 0.002) + "px";
    },
    contentSize() {
      ...
      if (n && this.showReset) {
        ...
        switch (this.type) {
          ...
          case "provincialStreet":
            ...
            this.areaDataList = areaData[defaultValue[0]][defaultValue[1]];
            this.streetDataList =
              streetData[defaultValue[0]][defaultValue[1]][defaultValue[2]];
            break;
          case "date":
            let data = QSPicker.countDays(
              this.years[defaultValue[0]],
              defaultValue
            );
            this.days = data.days;
            this.value = data.val;
  ...
  methods: {
      ...
      switch (this.type) {
        ...
        case "provincialStreet":
          ...
          if (this.value[0] !== changevalue[0]) {
            ...
            this.areaDataList = areaData[changevalue[0]][0];
            this.streetDataList =
              streetData[changevalue[0]] &&
              streetData[changevalue[0]][0] &&
              streetData[changevalue[0]][0][0]
                ? streetData[changevalue[0]][0][0]
                : [];
            changevalue[1] = 0;
            changevalue[2] = 0;
            ...
            this.streetDataList =
              streetData[changevalue[0]] &&
              streetData[changevalue[0]][changevalue[1]] &&
              streetData[changevalue[0]][changevalue[1]][0]
                ? streetData[changevalue[0]][changevalue[1]][0]
                : [];
            changevalue[2] = 0;
            changevalue[3] = 0;
            ...
            this.streetDataList =
              streetData[changevalue[0]] &&
              streetData[changevalue[0]][changevalue[1]] &&
              streetData[changevalue[0]][changevalue[1]][changevalue[2]]
                ? streetData[changevalue[0]][changevalue[1]][changevalue[2]]
                : [];
            changevalue[3] = 0;
          }
    ...
    confirm() {
      ...
      switch (type) {
        ...
        case "custom":
          ...
          if (setObj.linkage) {
            if (setObj.linkageNum === 2) {
              //二级联动
              data.steps1 = custom_datas[custom_val[0]];
              data.steps2 =
                custom_datas[custom_val[0]][custom_steps.step_2_item][
                  custom_val[1]
                ];
            } else if (setObj.linkageNum === 3) {
              //三级联动
              data.steps1 = custom_datas[custom_val[0]];
              if (!data.steps1)
                QSPicker.showToast("第一列中不存在第" + custom_val[0] + "项");
              data.steps2 =
                data.steps1[custom_steps.step_2_item][custom_val[1]];
              if (!data.steps2)
                QSPicker.showToast("第二列中不存在第" + custom_val[1] + "项");
              data.steps3 =
                data.steps2[custom_steps.step_3_item][custom_val[2]];
              if (!data.steps3)
          ...
          break;
        case "custom2":
          let custom2_datas = setObj.linkage
            ? setObj.itemObject
            : setObj.itemArray;
          ...
          if (setObj.linkage) {
            ...
            if (setObj.linkageNum === 2) {
              ...
              data.steps2 =
                custom2_datas.step_2[custom2_val[0]][custom2_val[1]];
            } else if (setObj.linkageNum === 3) {
              //三级联动
              data.steps1 = custom2_datas.step_1[custom2_val[0]];
              if (!data.steps1)
              ...
              data.steps3 =
                custom2_datas.step_3[custom2_val[0]][custom2_val[1]][
                  custom2_val[2]
                ];
              if (!data.steps3)
    ...
    _getprovincialStreetLabel() {
      let pcikerLabel =
        ...
        (this.streetDataList[this.value[3]]
          ? "-" + this.streetDataList[this.value[3]]
          : "");
      return pcikerLabel;
    },
