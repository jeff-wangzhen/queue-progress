        ...
        >
          <picker-view-column>
            <view
              ...
              :class="
                (value[0] || 0) === index ? 'selectColor' : 'defaultColor'
              "
          ...
          <picker-view-column>
            <view
              class="flex_row_c_c pickerDefault"
              :class="
                (value[1] || 0) === index ? 'selectColor' : 'defaultColor'
              "
              v-for="(item, index) in getItem2"
              :key="index"
            >
              {{ item[steps.step_2_value] || item }}
            </view>
          </picker-view-column>
        </picker-view>
      </block>
        ...
        >
          ...
          <picker-view-column>
            <view
              class="flex_row_c_c pickerDefault"
              :class="
                (value[2] || 0) === index ? 'selectColor' : 'defaultColor'
              "
              v-for="(item, index) in getItem3"
              :key="index"
            >
              {{ item[steps.step_3_value] || item }}
            </view>
          </picker-view-column>
        </picker-view>
      </block>
      ...
      >
        <picker-view-column v-for="(items, indexs) in itemArray" :key="indexs">
          <view
            class="flex_row_c_c pickerDefault"
            :class="
              (value[indexs] || 0) === index ? 'selectColor' : 'defaultColor'
            "
...
export default {
  ...
  computed: {
    getItem2() {
      if (this.async) {
        ...
        return this.steps &&
          this.steps.step_2_item &&
          this.itemArray[this.value[0] || 0]
    ...
    getItem3() {
      if (this.async) {
        ...
        if (
          ...
          return this.itemArray[this.value[0] || 0][this.steps.step_2_item][
            this.value[1] || 0
          ][this.steps.step_3_item];
        } else {
          return [];
  ...
  methods: {
    init() {
      if (
        this.dataSet.defaultValue instanceof Array &&
    ...
    getAsyncData(obj) {
      // 逐级获取
      let oldValue = [];
      if (this.oldValue.length && this.oldValue.length > 0)
      ...
      obj = {
        ...
        datas: {
          ...
          data_1:
            this.async_1.length && this.async_1.length > 0
              ? [...this.async_1]
              : [],
        },
    ...
    confirm() {
      ...
      if (_this.linkage) {
        ...
        if (_this.linkageNum == 2) {
          ...
          data.steps2 = async
            ? _this.async_1[v[1]]
            : datas[v[0]][steps.step_2_item][v[1]];
          ...
          data.steps2 = async
            ? _this.async_1[v[1]]
            : data.steps1[steps.step_2_item][v[1]];
          if (!data.steps2) _app.showToast("第二列中不存在第" + v[1] + "项");
          data.steps3 = async
            ? _this.async_2[v[2]]
            : data.steps2[steps.step_3_item][v[2]];
